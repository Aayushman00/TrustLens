"""Local Fairness dataset routes (upload/list/get/group-discovery).

Single-user local instance — no ownership concept; this path never
substitutes model or dataset, it evaluates the selected model against the
operator's own data.
"""
...
async def upload_dataset(
    ...
    row = _service(db, store).upload(filename=file.filename, data=data)
...
def list_datasets(
    ...
    rows = _service(db, store).list_all()
...
def get_dataset(
    ...
    row = _service(db, store).get_by_id(dataset_id)
    return UserDatasetRead.model_validate(row)
...
def get_dataset_groups(
    ...
    return _service(db, store).discover_groups(dataset_id, group_column=column)
...
def delete_dataset(
    ...
    _service(db, store).delete(dataset_id)
