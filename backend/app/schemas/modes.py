...
ASSISTED_REVIEWED_DISCLAIMER = (
    "Human review recorded, but at least one required aspect is missing a "
    "complete human O/S/D triple. O, S, and D are each independently "
    "human-entered — never inferred from probe evidence, never generated by "
    "a probe. FRIES is withheld until every required aspect has all three."
)
ASSISTED_REVIEWED_SCORED_DISCLAIMER = (
    "Human review recorded and complete for every required aspect. O, S, "
    "and D were each human-entered — never inferred from probe evidence, "
    "never generated by a probe (the deterministic mapper itself always "
    "abstains). FRIES has been scored from this human-supplied O/S/D."
)
...
SCORE_NOTE_DETERMINISTIC_SCORED = (
    "Original FRIES is computed from complete, human-entered O/S/D — not "
    "FRIES2. The deterministic mapper itself never proposes O/S/D; every "
    "value here was supplied by a human reviewer."
)
SCORE_NOTE_LEGACY = (
...
def disclaimer_for(
    ...
    *,
    ...
    if human_reviewed:
        if deterministic:
            # scoring_withheld is the already-authoritative completion signal
            # (mirrors count_scoring_complete_aspects/FRIES_ASPECT_COUNT via
            # the caller — see to_finalized_osd_assisted/get_mode_disclosure).
            # Only an explicit False (proven complete) selects the "scored"
            # text; None/True (unknown or genuinely incomplete) keeps the
            # existing withheld text — never a new completeness calculation.
...
def osd_provenance_bullet(
    *,
    ...
    if is_deterministic_engine(assessment_engine, methodology_status):
        if scoring_withheld is False:
            return (
                "O/S/D were human-entered for every required aspect — the "
                "deterministic mapper itself never proposes O/S/D. Not an "
                "LLM assessment."
            )
        return (
