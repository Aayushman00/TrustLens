...
def _create_and_run(
    ...
    *,
    ...
    model = api_client.post(
        "/v1/models",
        json=fries_complete_model_payload(f"org/leaderboard-{uuid.uuid4().hex[:8]}"),
        headers=headers,
    )
    ...
    created = api_client.post("/v1/evaluations", json=body, headers=headers)
...
def test_publish_appears_unpublish_disappears(
    ...
    eval_id = _create_and_run(
        api_client, admin_headers, db_session, mode="AI_AUTONOMOUS", task="sentiment"
    )

    published = _publish(api_client, admin_headers, eval_id)
    assert published["is_published"] is True
    assert published["published_at"] is not None
    first_published_at = published["published_at"]

    # Idempotent republish keeps the original stamp.
    again = _publish(api_client, admin_headers, eval_id)
    ...
    unpublished = api_client.post(
        f"/v1/evaluations/{eval_id}/unpublish", headers=admin_headers
    )
    ...
    repeat = api_client.post(f"/v1/evaluations/{eval_id}/unpublish", headers=admin_headers)
...
def test_task_filter_excludes_other_tasks(
    ...
    id_a = _create_and_run(
        api_client, admin_headers, db_session, mode="AI_AUTONOMOUS", task="task-a"
    )
    id_b = _create_and_run(
        api_client, admin_headers, db_session, mode="AI_AUTONOMOUS", task="task-b"
    )
    _publish(api_client, admin_headers, id_a)
    _publish(api_client, admin_headers, id_b)
...
def test_report_ref_attached_when_report_exists(
    ...
    eval_id = _create_and_run(
        api_client, admin_headers, db_session, mode="AI_AUTONOMOUS", task="with-report"
    )
    _publish(api_client, admin_headers, eval_id)
...
def test_cursor_pagination_smoke(
    ...
    for _ in range(3):
        eval_id = _create_and_run(
            api_client, admin_headers, db_session, mode="AI_AUTONOMOUS", task="paginate"
        )
        _publish(api_client, admin_headers, eval_id)
